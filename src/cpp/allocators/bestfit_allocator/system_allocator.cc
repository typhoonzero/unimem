...
static double fraction_of_gpu_memory_to_use = 0.92;
#endif

static bool use_pinned_memory = false;
...
void GPUAllocator::Free(void* p, size_t size, size_t index) {
  ...
  if (index == 0) {
    assert(gpu_alloc_size_ >= size);
    gpu_alloc_size_ -= size;
    err = cudaFree(p);
  } else {
    assert(fallback_alloc_size_ >= size);
...
void CUDAPinnedAllocator::Free(void* p, size_t size, size_t index) {
  ...
  assert(cuda_pinnd_alloc_size_ >= size);
  cuda_pinnd_alloc_size_ -= size;
  err = cudaFreeHost(p);
